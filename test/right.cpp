...
TEST(expression, test_right)
{
...

    {
        ...
        BOOST_MPL_ASSERT(
            (std::is_same<decltype(yap::right(plus_expr)), term<int> const &>));
    }

    {
...

        {
            ...
            BOOST_MPL_ASSERT((
                std::
                    is_same<decltype(yap::right(plus_expr_ref)), term<int> &>));
        }
...
TEST(user_expr, test_right)
{
...

    {
        plus_expr_type plus_expr = unity + user_term<int>{{1}};
        BOOST_MPL_ASSERT(
            (std::is_same<decltype(yap::right(plus_expr)), user_term<int> &>));
    }
