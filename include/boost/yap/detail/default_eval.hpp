...
#undef BOOST_YAP_BINARY_OPERATOR_CASE
    ...
    // TODO: Document that nonterminal subexpressions in a default-transformed
    // expression must have been made from an ExpressionTemplate, or
    // transform() chokes right here.
    template<
    ...
    decltype(auto) default_transform_nonterminal(
    ...
    struct default_transform<true, false>
    {
        ...
        decltype(auto) operator()(Expr && expr, Transform & transform) const
        {
            return default_transform_nonterminal(
                expr, expr.elements, transform);
        }
    };
    ...
    operator()(Expr && expr, Transform & transform)
    {
        ...
        auto xform = hof::unpack(hof::first_of)(
            // For each transform...
            std::make_pair(
                // First, try to use a tag-form.
                transform_expression_tag<
                    Expr,
                    Transform,
                    detail::arity_of<kind>()>{},
                // Next, try to use an Expression-form.
                transform_expression_expr<Expr, Transform>{}),
            std::make_tuple(
                // If all else fails, just forward a terminal, or return a
                // non-terminal with its elements recursively transformed.
                default_transform<
                    std::is_lvalue_reference<Expr>{},
                    kind == expr_kind::terminal>{}));
        return xform(static_cast<Expr &&>(expr), transform);
    }
